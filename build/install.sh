...
Now you should access CLI script: $ galaxy-mk2 --help
Alternatively you can access CLI script via poetry run: $ poetry run galaxy-mk2 --help
To deactivate virtualenv simply type: $ deactivate
To activate shell completion:
 - for bash: $ echo 'eval "$(_GALAXY_MK2_COMPLETE=source_bash galaxy-mk2)' >> ~/.bashrc
 - for zsh: $ echo 'eval "$(_GALAXY_MK2_COMPLETE=source_zsh galaxy-mk2)' >> ~/.zshrc
 - for fish: $ echo 'eval "$(_GALAXY_MK2_COMPLETE=source_fish galaxy-mk2)' >> ~/.config/fish/completions/galaxy-mk2.fish
EOF