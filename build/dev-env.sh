...
minikube image load "galaxy-mk2:${TAG}"

echo "[dev-env] creating galaxy-mk2 namespace"
kubectl create namespace galaxy-mk2

ATTEMPTS=0
ROLLOUT_STATUS_CMD="kubectl get namespace galaxy-mk2 -n galaxy-mk2"
...
kubectl create -f manifests/persistent-storage-no-pvc-deletion.yaml -n galaxy-mk2

ATTEMPTS=0
ROLLOUT_STATUS_CMD="kubectl rollout status deployment.apps/rfs-redisfailover-persistent-keep -n galaxy-mk2"
until $ROLLOUT_STATUS_CMD || [ $ATTEMPTS -eq 60 ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  sleep 10
done
...
STATEFULSET_REPLICAS=$(kubectl get statefulset rfr-redisfailover-persistent-keep -o jsonpath='{.spec.replicas}' -n galaxy-mk2)
ATTEMPTS=0
until [[ ${STATEFULSET_REPLICAS} -eq $(kubectl get statefulset rfr-redisfailover-persistent-keep -o jsonpath='{.status.readyReplicas}' -n galaxy-mk2) ]] || [ $ATTEMPTS -eq 60 ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  sleep 10
done

echo "[dev-env] installing galaxy-mk2 charts"
helm upgrade --install \
    galaxy-mk2 charts/galaxy-mk2 \
    --namespace galaxy-mk2 \
    --set ingress.host.name="galaxy-mk2.${INGRESS_HOST}"

ATTEMPTS=0
ROLLOUT_STATUS_CMD="kubectl rollout status deployment/galaxy-mk2 -n galaxy-mk2"
until $ROLLOUT_STATUS_CMD || [ $ATTEMPTS -eq 60 ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  sleep 10
done
...
FastAPI available under: http://galaxy-mk2.${INGRESS_HOST}/
